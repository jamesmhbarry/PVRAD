...
def generate_disort_files(rt_config,home,cosmo_path,modis_path,itime,
                          grid,data,albedo,i_aod,aod_sample,sens_type): #cloudfile,
    """
...
def read_disort_output(grid,output_file):
    ...
    :return Idiff.values: array with radiance field distribution for one time step
    """    
...
def disort_simulation_aod(dataframe,rt_config,grid,angles,
                          albedo_series,modis_path,cosmo_path,sens_type,home): #cloudfile,
    """    
...
def save_results(name,pv_station,name_df,info,rt_config,pyr_config,path):
    """
...
if args.configfile:
    ...
    config_filename = "config_PVPYROD_MetPVNet_messkampagne.yaml"
...
for campaign in campaigns:
    ...
    for key in pvsys:    
        print('Simulating clear sky radiative transfer for %s, %s using DISORT and wavelength range %s' 
        ...
        #Prepare data for aod simulation        
        ...
        if name_df_sim in pvsys[key]:  
            ...
            print(f'Clear days are {[day.strftime("%Y-%m-%d") for day in rt_config["test_days"][year]["clear"]]}')
            
            name_df_aod = "df_aod_" + year.split('_')[-1]    
            pvsys[key][name_df_aod] = disort_simulation_aod(df_temp,
                                 rt_config,grid_dict,
                                 angle_arrays,albedo_series,
                                 modis_path,cosmo_path,sensor_type,homepath) #cloudfile,
           
            save_results(key,pvsys[key],name_df_aod,campaign,rt_config,pyr_config,savepath)
