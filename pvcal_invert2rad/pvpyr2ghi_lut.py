...
# def calc_statistics_ghi(key,pv_station,year,substat,substat_type,timeres,
#                         pvrad_config,pyrcal_config,folder,senstype):
#     """
...
#     dfname = f'df_{senstype}_ghi_' + year.split('_')[-1] + '_' + timeres
...
#     delta_GHI = dataframe[("Etotdown_lut_Wm2",substat)]\
#         - dataframe[(radname,pyrname)]
#     rmse = (((delta_GHI)**2).mean())**0.5
#     mad = abs(delta_GHI).mean()
#     mbe = delta_GHI.mean()
    
#     delta_max_plus = delta_GHI.max()
#     delta_max_minus = delta_GHI.min()                
    
#     n_delta = len(delta_GHI.dropna())
    
#     stats.update({substat:{}})
    
#     stats[substat].update({"n_delta":n_delta})
#     stats[substat].update({f"RMSE_GHI_{pyrname}_Wm2":rmse})
#     stats[substat].update({f"MAD_GHI_{pyrname}_Wm2":mad})
#     stats[substat].update({f"MBE_GHI_{pyrname}_Wm2":mbe})
#     stats[substat].update({f"max_Delta_GHI_plus_{pyrname}_Wm2":delta_max_plus})
#     stats[substat].update({f"max_Delta_GHI_minus_{pyrname}_Wm2":delta_max_minus})
    
#     print(f"{key}, {yrname}: statistics at {timeres} calculated with {n_delta} measurements")
#     print(f"RMSE for GHI inverted from {substat} compared to {pyrname} is {rmse}")
#     print(f"MAE for GHI inverted from {substat} compared to {pyrname} is {mad}")
#     print(f"MBE for GHI inverted from {substat} compared to {pyrname} is {mbe}")
    
#     #Assign delta to the dataframe
#     pv_station[dfname].loc[dataframe.index,("delta_GHI_Wm2",substat)] = delta_GHI
    
#     if f"df_stats_{timeres}_{yrname}" not in pv_station:
#         pv_station[f"df_stats_{timeres}_{yrname}"] = pv_station[dfname].loc[dataframe.index,[("delta_GHI_Wm2",substat),
...
#                 pv_station[dfname].loc[dataframe.index,[("delta_GHI_Wm2",substat),
#                                         ("Etotdown_lut_Wm2",substat),(radname,pyrname)]]],axis=1)            
...
#     # else: df_stats_final = pd.DataFrame()
    ...
    # return df_stats #df_stats_final
                ...
                for substat in pvsys[key]["substations_pv"][substat_type]["data"]:    
                    ...
                    if "poa" in radname:                            
                        ...
                        # df_temp = pvsys[key][name_df_hires] #
                            ...
                            #     combo_stats[year][f"df_{yrname}_stats"] = 
                            
                            # dfs_stats.append(calc_statistics_ghi(key,pvsys[key],
                            #           year,substat,substat_type,timeres,pvrad_config,
                            #           pyr_config,savepath_pyr,'pv'))                                                        
                            # # else:
                            #     combo_stats[year][f"df_{yrname}_stats"] = pd.concat([combo_stats[year][f"df_{yrname}_stats"],
                            #             calc_statistics_ghi(key,pvsys[key],year,substat,substat_type,timeres,pvrad_config,
                            #                 pyr_config,savepath_pyr,'pv')],axis=0)
            
            # if len(dfs_stats) != 0:
            #     if combo_stats[year][f"df_{yrname}_stats"].empty:
            #         combo_stats[year][f"df_{yrname}_stats"] = pd.concat(dfs_stats,axis=1)
            #     else:
            #         combo_stats[year][f"df_{yrname}_stats"] = pd.concat([combo_stats[year][f"df_{yrname}_stats"],
            #                                                pd.concat(dfs_stats,axis=1)],axis=0)
            #     for timeres in pvrad_config["timeres_comparison"]:
            #         if f"df_stats_{timeres}_{yrname}" in pvsys[key]:
            #             pvsys[key][f"df_stats_{timeres}_{yrname}"].columns.rename(["variable","substat"],inplace=True)                    
            #             idx = pvsys[key][f"df_stats_{timeres}_{yrname}"].columns.to_frame()
            #             idx.insert(2, 'station', key)
            #             pvsys[key][f"df_stats_{timeres}_{yrname}"].columns = pd.MultiIndex.from_frame(idx) 
                        
            #             new_cols = [col for col in pvsys[key][f"df_stats_{timeres}_{yrname}"].columns.levels[1]\
            #                         if ("Pyr" not in col) and ("CMP" not in col) and ("RT" not in col)]            
            #             if timeres == "1min" and key != "MS_02":
            #                 new_cols = [col.split('_')[0] for col in new_cols if ("egrid" in col)]
            #             # elif timeres == "15min":
            #             #     new_cols = [col for col in new_cols if "auew" in col]
            #             pvsys[key][f"df_stats_{timeres}_{yrname}"].columns = pd.MultiIndex.from_product(
            #                 [new_cols,['delta_GHI_Wm2','GHI_PV_inv','GHI_Pyr_ref'],[key]],
            #                 names=['substat','variable','station']).swaplevel(0,1)
            #         #else: combo_stats[year][f"df_{yrname}_stats"] = pd.DataFrame()
