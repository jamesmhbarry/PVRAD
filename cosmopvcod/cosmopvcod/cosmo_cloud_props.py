...
def load_data(path_cosmo, fname_cosmo, foldername, vardict):
    ...
    for key in ["cod_water","cod_ice","cod","cloud_fraction"]:
...
def get_cosmo_filenames(path, dates):
    ...
    for d in dates_str:
        for folder in folders:
            if d in folder:  
                file_dict.update({folder:[]})
                files = list_files(os.path.join(path,folder))
                for f in files:
                    file_dict[folder].append(f)
...
def generate_new_time_array(times, timeunit, timeres, datatype):
    ...
    :param datatype: string with datatype
...
def write_cod_file(path, fname, data_dict, lat_lon_idx, time, header, grid_avg):
    """
    ...
    hdr_add = f"Optical depth at 600nm of water and ice clouds from COSMO, matched to nearest grid point, mean and iqr from {grid_avg}"
    
    header_new = "{}\n{:^20} {}".format(hdr_add, "time", "{:>15} {:>15} {:>15} {:>10} {:>20} {:>20} {:>20} {:>20} {:>15}"
    ...
    cod_water = data_dict["cod_water"][:,lat,lon]
    cod_ice = data_dict["cod_ice"][:,lat,lon]        
    cod_tot = data_dict["cod"][:,lat,lon]        
    cf_tot = data_dict["cloud_fraction"][:,lat,lon]    
    
    #Weight the cloud optical depth with cloud fraction    
    cod_tot_weighted = data_dict["cod"]*data_dict["cloud_fraction"]    
    
    #Old method, add water and ice COD
    cod_tot_wi_weighted = (data_dict["cod_water"] + data_dict["cod_ice"])*data_dict["cloud_fraction"]    
            
    #This code will not work for the edge of the model domain!!, 23.07.2021
    cf_mean = np.mean(data_dict["cloud_fraction"][:,lat-dlat:lat+dlat+1,lon-dlon:lon+dlon+1],axis=(1,2))
    
    #Old method using sum of water and ice
    cod_tot_wi_mean = np.mean(cod_tot_wi_weighted[:,lat-dlat:lat+dlat+1,lon-dlon:lon+dlon+1],axis=(1,2))/cf_mean
    cod_tot_wi_iqr = np.subtract(*np.percentile(cod_tot_wi_weighted[:,lat-dlat:lat+dlat+1,lon-dlon:lon+dlon+1],
                                                [75, 25],axis=(1,2)))/cf_mean
    
    #New method using new calculation, 11.08.2021
    cod_tot_mean = np.mean(cod_tot_weighted[:,lat-dlat:lat+dlat+1,lon-dlon:lon+dlon+1],axis=(1,2))/cf_mean
    cod_tot_iqr = np.subtract(*np.percentile(cod_tot_weighted[:,lat-dlat:lat+dlat+1,lon-dlon:lon+dlon+1],
                                                [75, 25],axis=(1,2)))/cf_mean
    
    save_data = np.column_stack(([dt.strftime(" %d.%m.%Y;%H:%M:%S") for dt in time],
                                 ["%15.3f" % codw for codw in cod_water],                                 
                                 ["%15.3f" % codi for codi in cod_ice],
                                 ["%15.3f" % cod for cod in cod_tot],
                                 ["%10.3f" % cf for cf in cf_tot],
                                 ["%20.3f" % cod for cod in cod_tot_mean],
                                 ["%20.3f" % cod for cod in cod_tot_iqr],
                                 ["%20.3f" % cod for cod in cod_tot_wi_mean],
                                 ["%20.3f" % cod for cod in cod_tot_wi_iqr],
                                 ["%15.3f" % cf for cf in cf_mean]
                                  ))    
...
def save_cloud_files(save_path, file_dict, folder_key, data_dict, idx_dict, lat_lon_gp, 
                     grid_avg, stations_lat_lon, time, header):
    """
    ...
    :param file_dict: dictionary with filenames
    :param folder_key: string, name of current folder
        ...
        if lat_lon_gp[key] in val_set:
            ...
            if mdir_exist[key]:
                ...
                if any(files_exist[key]):
                    ...
                    write_cod_file(path, filename, data_dict, idx_dict[key], time, header, grid_avg)
                else:
                    path = os.path.join(save_path, mdir_namedict[key])
                    write_cod_file(path, filename, data_dict, idx_dict[key], time, header,grid_avg)
...
def create_cod_files(path_cosmo, fname_cosmo, foldername, vardict, stations_lat_lon, save_path_cod,
                                    grid_avg, timeres=None):
    """
    ...
    """
    #Load data           
    data_dict = load_data(path_cosmo,fname_cosmo,foldername,vardict)
    
    #Extract surface data
    cloud_dict = {k: data_dict[k] for k in ("cod_water","cod_ice","cod","cloud_fraction")}
    ...
    header_cod = ["tau_w_600","tau_i_600","tau_tot_600","cf_tot",
                  "mean_tau_tot_600","iqr_tau_tot_600",
                  "mean_tau_tot_wi_600","iqr_tau_tot_wi_600","mean_cf_tot"]        
    ...
    if timeres != None:
        ...
        save_cloud_files(save_path_cod, fname_cosmo, foldername, cloud_dict, idx_nearest_gp, lat_lon_nearest_gp, 
                       grid_avg, stations_lat_lon, data_dict["time"], header_cod)
...
def main():
    ...
    config_filename = os.path.abspath(args.configfile) # "../examples/config_COSMO2PVCODinput_2018_messkampagne.yaml" # # # #
    ...
    fnames_cosmo   = config["foldernames_cosmo"]
    ...
    for foldername in fnames_cosmo:
        create_cod_files(path_cosmo, fnames_cosmo, foldername, vardict_cosmo, stations_lat_lon, 
                                        save_path_cod, grid_avg, timeres=timeres)
